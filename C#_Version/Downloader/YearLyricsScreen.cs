...
namespace Downloader
{
    public partial class YearLyricsScreen : UserControl
    {
        ...
        private Dictionary<string, List<SemaphoreSlim>> AlbumsBeingChecked;
        ...
        public YearLyricsScreen(DownloaderForm window, List<string> newFiles)
        {
            ...
            try
            {
                Process.GetProcessesByName("py")[0].Kill();
            }
...
#warning TODO: When one thread finishes, rearrange files per threads
        ...
        private void StartThreads()
        {
            ...
            this.AlbumsBeingChecked = new Dictionary<string, List<SemaphoreSlim>>();
        ...
        private void AddToOutput(string artist, string album, string title, int threadIndex)
        {
            lock (Mutex)
            {
                ...
                Action update = () =>
                {
                    this.richTextBoxArtist.AppendText((line > 0 ? Environment.NewLine : "") + artist);
                ...
                update = () =>
                {
                    this.richTextBoxAlbum.AppendText((line > 0 ? Environment.NewLine : "") + album);
                ...
                update = () =>
                {
                    this.richTextBoxTitle.AppendText((line > 0 ? Environment.NewLine : "") + title);
            ...
            lock (Mutex)
            {
                ...
                else
                {
                    ...
                    update = () =>
                    {
                        ...
                        this.textBoxThreadStatus.SelectedText = "Thread " + (threadIndex + 1) + ": " + filesProcessed + "/" + totalFiles + " Files Processed";
                        this.textBoxThreadStatus.Select(this.textBoxThreadStatus.GetFirstCharIndexFromLine(NUMBER_OF_THREADS), this.textBoxThreadStatus.Lines[NUMBER_OF_THREADS].Length);
            ...
            while (true)
            {
                lock (Mutex)
                {
                    if (!this.PagesVisited_Year.ContainsKey(artist + album) && this.WorkerIndexError != threadIndex)
                    {
                        ...
                        {
                            this.AlbumsBeingChecked[initialAlbumTitle].Add(new SemaphoreSlim(0, 1));
                            indexSem = this.AlbumsBeingChecked[initialAlbumTitle].Count - 1;
                            semWait = true;
                        }
                        else
                        {
                            releaseSems = true;
                            this.AlbumsBeingChecked.Add(initialAlbumTitle, new List<SemaphoreSlim>());
                        }
                    }
                }
                if (semWait)
                {
                    this.AlbumsBeingChecked[initialAlbumTitle][indexSem].Wait();
                    this.ChangeArtistAlbumTitle(true, ref artist, ref album, ref title, trackCount);
                }
        ...
        public void GetLyricsAndYear(object parameters)
        {
            ...
            foreach (string filename in list)
            {
                ...
                //this.AlbumsBeingChecked[threadIndex] = currentAlbum;
